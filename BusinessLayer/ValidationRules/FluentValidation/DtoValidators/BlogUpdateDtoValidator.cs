...
namespace BusinessLayer.ValidationRules.FluentValidation.DtoValidators
{
    public class BlogUpdateDtoValidator : AbstractValidator<BlogUpdateDto>
    {
        ...
        {
            RuleFor(x => x.Id).NotEmpty().WithMessage(_validatorMessages.NotEmpty);


            RuleFor(x => x.Title).NotEmpty().WithName("Başlık").WithMessage("{PropertyName}" + _validatorMessages.NotEmpty).MaximumLength(100).WithMessage("{PropertyName} {MaxLength}" + _validatorMessages.NotBigger).MinimumLength(5).WithMessage("{PropertyName} {MinLength}" + _validatorMessages.NotSmaller);
            ...
            RuleFor(x => x.Thumbnail).NotEmpty().WithName("Thumbnail").WithMessage("{PropertyName}" + _validatorMessages.NotEmpty).MaximumLength(250)
            ...
            RuleFor(x => x.Image).MaximumLength(100).WithName("Resim").WithMessage("{PropertyName} {MaxLength}" + _validatorMessages.NotBigger);
            ...
            RuleFor(x => x.SeoDescription).NotEmpty().WithName("Seo Açıklama Bilgisi").WithMessage("{PropertyName}" + _validatorMessages.NotEmpty).MaximumLength(150)
                .WithMessage("{PropertyName} {MaxLength}" + _validatorMessages.NotBigger).MinimumLength(0).WithMessage("{PropertyName} {MinLength}" + _validatorMessages.NotSmaller);
            ...
            RuleFor(x => x.IsDelete).NotEmpty().WithName("Silindi Mi?").WithMessage("{PropertyName}" + _validatorMessages.NotEmpty);
